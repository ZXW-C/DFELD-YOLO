...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is GhostSEC3:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            ...
            if len(args) >= 1:
                ...
                base_c2 = c1
            reduction_ratio = args[1] if len(args) >= 2 else 16
            spd_scale = args[2] if len(args) >= 3 else 2
            c2 = make_divisible(min(base_c2, max_channels) * width, 8)
            args = [c1, c2, reduction_ratio, spd_scale]
        else:
            c2 = ch[f]
